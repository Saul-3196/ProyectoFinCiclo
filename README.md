...
🔍 Buscador de Rutas con Filtros: Panel interactivo para buscar rutas filtrando por localidad, nivel de dificultad (baja, media, alta, extrema) y modalidad de bicicleta[cite: 135, 136, 140].
🗺️ Integración con Google Maps: Visualización detallada del trazado de la ruta en el mapa a partir de coordenadas comprimidas (*Polylines*), incluyendo la marca del punto de encuentro[cite: 92, 103, 169].
👥 Sistema de Inscripción: Contador en tiempo real (`Apuntados`) que muestra el número y los nombres de los participantes que se han unido a la grupeta para esa salida[cite: 28].
...
2. Backend (API REST):** Desarrollado en PHP, encargado de recibir los parámetros de la app, procesar la lógica de negocio y comunicarse con el almacenamiento[cite: 48].
